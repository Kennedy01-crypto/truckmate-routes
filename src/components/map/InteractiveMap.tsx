...
export const InteractiveMap = ({
  ...
  useEffect(() => {
    ...
    locations.forEach(location => {
      ...
      const marker = new mapboxgl.Marker(el).setLngLat([location.lng, location.lat]).setPopup(new mapboxgl.Popup().setHTML(`<div style="color: black;"><strong>${location.type.charAt(0).toUpperCase() + location.type.slice(1)}</strong><br/>${location.address}</div>`)).addTo(map.current!);
  ...
  if (needsToken && !mapboxToken) {
    return <div className={cn("eld-map-container relative flex items-center justify-center", className)} style={{
      minHeight: '300px'
    }}>
        
      </div>;
  }
  return <div className={cn("eld-map-container relative", className)}>
      {/* Loading Overlay */}
      {isLoading && <div className="absolute inset-0 bg-eld-map-bg/90 backdrop-blur-sm flex items-center justify-center z-10">
      ...
      {/* Map Canvas */}
      <div ref={mapContainer} className="w-full h-full relative overflow-hidden" style={{
      minHeight: '300px'
    }} />

      {/* Interaction Feedback */}
      {mapInteraction && <div className="absolute top-4 left-4 bg-card border border-border rounded-lg px-3 py-2 animate-slide-up z-10">
